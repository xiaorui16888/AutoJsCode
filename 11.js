...
log(" 1.存款利息计算器\n 2.贷款利息计算器"); //提示选择计算器
log("请选择:");
var xzlx = console.rawInput().toString(10); //输入选项 
if (xzlx == 1) //如果输入数字选项 1 
{
    log("欢迎使用存款利息计算器！"); //显示欢迎语 
    log("请输入存款金额:"); //输入存款金额 
    var jine = console.rawInput().toString(10); //输入金额
    log("======存款期限====== \n1.活期存款\n 2.3个月\n 3.6个月\n 4.一年\n 5.两年\n 6.三年\n 7.五年");
    log("请输入存款期限的代号："); //输出存款类型 
    var xzlx2 = console.rawInput().toString(10); //输入选项 
    if (xzlx2 >= 1 && xzlx2 <= 7) //如果输入数字选项	大于等于1且小于等于7 
    {
        if (xzlx2 == 1) {
            log("请输入存款天数："); //提示信息 
            var tians = console.rawInput().toString(10); //输入天数 
